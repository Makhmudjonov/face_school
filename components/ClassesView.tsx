...
import { Search, Plus, Edit, Trash2, Users, BookOpen, X, Save, Loader2, Building2 } from 'lucide-react';
...
export const ClassesView: React.FC = () => {
  ...
  const fetchClasses = async () => {
    ...
    try {
        ...
        const response = await fetch('https://hik-test.tashmeduni.uz/api/v1/settings/classroom/list/', {
            headers: { 'Authorization': `Bearer ${token}` }
        });

        if (response.ok) {
            const data = await response.json();
            const mappedClasses = data.data.map((item: any) => ({
                id: item.id,
                name: item.name,
  ...
  const handleOpenModal = (cls?: ClassGroup) => {
      ...
      if (cls) {
          ...
          setFormData({
              name: cls.name,
              school: cls.school ? cls.school.toString() : '',
          ...
          setFormData({ name: '', school: defaultSchool, teacherName: '', room: '' });
      }
      setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
          const token = localStorage.getItem('access_token');
          const isUpdate = !!currentClass;
          const url = isUpdate 
            ? `https://hik-test.tashmeduni.uz/api/v1/settings/classroom/update/${currentClass.id}/`
            : 'https://hik-test.tashmeduni.uz/api/v1/settings/classroom/create/';
        ...
        {isLoading ? (
            ...
            <table className="w-full text-left border-collapse">
                ...
                {filteredClasses.map((cls) => {
                    const schoolObj = schools.find(s => s.id === cls.school);
                    return (
                    <tr key={cls.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                    <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-lg bg-blue-50 dark:bg-blue-900/30 flex items-center justify-center text-blue-600 dark:text-blue-400">
                                <BookOpen className="w-4 h-4" />
                            </div>
                            <span className="font-bold text-slate-800 dark:text-white">{cls.name}</span>
                        </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                            <button onClick={() => handleOpenModal(cls)} className="p-2 text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg">
                                <Edit className="w-4 h-4" />
                            </button>
                            <button onClick={() => handleDelete(cls.id)} className="p-2 text-slate-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-700/50">
              <h3 className="text-lg font-bold text-slate-800 dark:text-white">{currentClass ? 'Tahrirlash' : 'Qo\'shish'}</h3>
